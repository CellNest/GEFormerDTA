...
class GEFormerDTA_with_Edge(torch.nn.Module):
    ...
    def forward(self, drug, prot, max_d_node=256, multi_hop_max_dist=20, spatial_pos_max=20):
        drug_node, drug_attn_bias, drug_edge_input = [], [], []
        x, edge_index, batch, d_node, d_attn_bias, d_edge_input = drug.x, drug.edge_index, drug.batch, drug.node, drug.attn_bias, drug.edge_input
...
# ------------------------------------------------------------------------------------------------------------
        ...
        #                                                                                                2)  # 2,8,43,43
        drug_graph_attn_bias = drug_graph_attn_bias + drug_edge_input.permute(0, 4, 3, 1, 2)
