...
class GEFormerDTA_with_SpatialP(torch.nn.Module):
    ...
    def forward(self, drug, prot, max_d_node=256, multi_hop_max_dist=20, spatial_pos_max=20):
        drug_node, drug_attn_bias, drug_spatial_pos = [], [], []
        x, edge_index, batch, d_node, d_attn_bias, d_spatial_pos = drug.x, drug.edge_index, drug.batch, drug.node, drug.attn_bias, drug.spatial_pos
...
# ------------------------------------------------------------------------------------------------------------
        ...
        #                                  :max_dist, :, :])
        # drug_edge_input = drug_edge_input_flat.reshape(max_dist, drug_n_graph, drug_n_node, drug_n_node,
        #                                                self.num_heads).permute(1, 2, 3, 0, 4)  # 2,43,43,20,8
        # drug_edge_input = (drug_edge_input.sum(-2) / (drug_spatial_pos.float().unsqueeze(-1))).permute(0, 3, 1, 2)  # 2,8,43,43
